...
public class videoPlayback {
	...
	public static void main(String[] args) {
		 ...
		 * " Please pass video and audio filename "); return; }
		 */
		...
		final Runnable r1 = new Runnable() {
			public void run() {
				...
				try {
					...
					/*
					 * Size of each frame is 240 * 320 * 3 (1 byte each for R,
					 * G, B)
					 */
					long len = 270 * 480 * 3;
					...
					long start, stop;
					/*
					 * The video framerate is 24 fps means around 1000 / 24 =
					 * 41.67
					 */
					long delay_val = 67;
					...
					for (i = 1; i < file.length() / bytes.length; i++) {
						...
						/*
						 * current_video_frame is not equal to
						 * current_expected_frame if video is lagging, drop
						 * (skip) some frames to compensate the lag
						 * 
						 * if video is leading over audio, wait to compensate
						 * the lag
						 */
						if (current_video_frame != current_expected_frame) {
							...
							/*
							 * video is leading so wait till the lag is
							 * compensated by audio
							 */
						...
						stop = System.currentTimeMillis();
						/*
						 * As the video fps = 24 and actual delay value is 41.67
						 * and we are considering 41, we have to wait 2
						 * milliseconds extra for every 3rd frame
						 */
						if (i % 3 == 0) {
...
/*
 * class for playing audio extending video
 */
